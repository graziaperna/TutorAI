...
askablemenu(questions_topic(X,Y), 'Al termine dello studio chiedi ad altre persone di porti delle domande sull’argomento studiato?',[si, no, a volte],questions_topic,X,Y).
...
askablemenu(deepening(X,Y), 'Quando studi un argomento di solito sei interessato ad approfondirlo?',[si, no, a volte],deepening,X,Y).
...
askablemenu(computer(X,Y), 'Quanto spesso usi il computer?',[ogni giorno, ogni tre giorni, nessuno tra queste],computer,X,Y).
...
askablemenu(notes(X,Y), 'Durante la lezione, preferisci seguire attivamente ciò che dice l’insegnante o prendere appunti?',[seguire attivamente, prendere appunti],notes,X,Y).
askablemenu(movie(X,Y), 'Ti piacciono i film tratti da storie/situazioni vere?',[si, no],movie,X,Y).
askablemenu(group_study(X,Y), 'Preferisci studiare da solo o in compagnia?',[da solo, in compagnia],group_study,X,Y).
askablemenu(videogames(X,Y), 'Trovi siano utili i videogiochi didattici?',[si, no],videogames,X,Y).
askablemenu(books_quantity(X,Y), 'Quanti libri leggi?',[almeno 1 al mese, uno ogni 2 mesi, uno ogni 6 mesi, nessuno tra queste],books_quantity,X,Y).
askablemenu(guided_visits(X,Y), 'Trovi interessanti le visite guidate?',[si, no],guided_visits,X,Y).
askablemenu(group_problem(X,Y), 'Se un membro del team iniziasse a prendersi il merito dei tuoi contributi, cosa faresti?',[cercherei di risolvere la situazione, cercherei di cambiare team],group_problem,X,Y).
askablemenu(group_sports(X,Y), 'Preferisci sport di gruppo o individuali?',[di gruppo, individuali],group_sports,X,Y).
askablemenu(group_divergences(X,Y), 'Se tu e un membro del team non siete d’accordo su come procedere con un progetto di gruppo, come si arriva a una decisione?',[ne discutiamo in modo costruttivo valutando i pro e i contro, non ascolto cosa gli altri hanno da dire, subito capisco che ho torto],group_divergences,X,Y).
askablemenu(notes_type(X,Y), 'Preferisci prendere appunti sul quaderno o sul computer?',[quaderno, computer],notes_type,X,Y).
askablemenu(lessons_type(X,Y), 'Preferisci seguire le lezioni online o in presenza?',[online, in presenza],lessons_type,X,Y).
askablemenu(exercise_place(X,Y), 'Preferisci svolgere gli esercizi in classe o a casa?',[in classe, a casa],exercise_place,X,Y).
...
rule(sl_dim,method(X,'Slide e dimostrazione'),and([is(X,multimedia_doc), is(X,key_doc),is(X,subject_experience), is(X,video_explanation), is(X,audio_explanation)]),1,1).
...
rule(supported_learning_1,is(X,supported_learning),and([is(X, guided_learning), is(X, practical_learning), or([is(X, group_learning), is(X, multimedia)])]),1,1).
...
rule(multimedia_6,is(X,multimedia),and([notes(X, prendere appunti), notes_type(X, computer)]),1,1).
...
rule(document_3,is(X,document),or([deepening(X, si), deepening(X, a volte)]),1,1).
rule(document_4,is(X,document),or([books_quantity(X, almeno 1 al mese), books_quantity(X, uno ogni 2 mesi)]),1,1).
rule(document_5,is(X,document),and([notes(X, prendere appunti), notes_type(X, quaderno)]),1,1).
rule(sample_doc_1,is(X,sample_doc),and([notes(X, prendere appunti), colors(X, si)]),1,1).
...
rule(practical_learning_5,is(X,practical_learning),and([exercise_place(X, a casa)]),1,1).
...
rule(guided_learning_3,is(X,guided_learning),or([questions_topic(X, si), questions_topic(X, a volte)]),1,1).
...
rule(guided_learning_6,is(X,guided_learning),and([group_study(X, in compagnia)]),1,1).
rule(guided_learning_7,is(X,guided_learning),and([exercise_place(X, in classe)]),1,1).
rule(analitic_learning_1,is(X,analitic_learning),and([enigma(X, si)]),1,1).
rule(analitic_learning_2,is(X,analitic_learning),or([deepening(X, si), deepening(X, a volte)]),1,1).
...
rule(visual_5,is(X,visual),or([books_quantity(X, almeno 1 al mese), books_quantity(X, uno ogni 2 mesi)]),1,1).
rule(visual_6,is(X,visual),or([computer(X, ogni giorno), computer(X, ogni tre giorni)]),1,1).
...
rule(group_learning_1,is(X,group_learning),and([group_study(X, in compagnia)]),1,1).
rule(group_learning_2,is(X,group_learning),and([group_divergences(X, ne discutiamo in modo costruttivo valutando i pro e i contro)]),1,1).
rule(group_learning_3,is(X,group_learning),and([group_problem(X, cercherei di risolvere la situazione)]),1,1).
rule(group_learning_4,is(X,group_learning),and([group_sports(X, di gruppo)]),1,1).
rule(group_learning_5,is(X,group_learning),and([questions_topic(X, si), questions_topic(X, a volte)]),1,1).
