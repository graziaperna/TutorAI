/*************************************************************************
	...
	Descrizione:
	   ...
	   Obiettivo:
		...
		Obiettivo:
			- Inserimento ":- dynamic (modulo_corrente/1)." per prevenire errori di caricamento del modulo in alcune versioni di Prolog.
			- Possibilit� di inibire "fireset_do(Idr,[R|L],C,P,Crt)" per impedire la duplicazione delle regole nel caso di clausole multiple nella testa.
			- Possibilit� di asserire fatti multipli appositamente inseriti in una lista.x
...
mainloop :-
	...
	fact(_,_,_),
	mainloop.
% Modifica Iovine-Lovascio per la gestione del caricamento e scaricamento differito dei moduli
mainloop :-
	not(fact(_,_,_)),
	current_predicate(module_to_be_loaded/1), %%modifica occhionero - mennitti
	...
	!.
% Modifica F. Galetta
% ripristinata la possibilità asserire tutti i fatti inferiti
mainloop_complete :-
	fact(_,_,_),	
	forward,		
	gui_mainloop(forward_finished),	
...
manage_delayed_modules :-
	current_predicate(module_to_be_loaded/1), %% modifica mennitti-occhionero
	module_to_be_loaded(ModPath),
	assert(modulo_corrente(ModPath)),
	load(ModPath),
	retractall(module_to_be_loaded(ModPath)),
	manage_delayed_modules.
manage_delayed_modules :-
	current_predicate(module_to_be_unloaded/1), %% modifica mennitti-occhionero
...
%%modifica mennitti_occhionero per eliminare solo i fatti del 
%modulo corrente
retract_fact:-  
		modulo_corrente(ModName),
		retractall(fact(id(ModName,_),_,_)),nl,
		write('     Cleaning fact finished'),nl,
		retract(modulo_corrente(ModName)).
retract_fact:- 
		modulo_iniziale(Iniziale),
		modulo_corrente(ModName),
		Iniziale == ModName,			
		retract(modulo_corrente(ModName)).
...
retract_used_fact:-
		   modulo_corrente(ModName),
	      	   retractall(used_fact(id(ModName,_),_,_)),nl,
	      	   write('     Cleaning fact finished'),nl,
	      	   retract(modulo_corrente(ModName)).
...
mainloop :-
	...
	(stricomm -> write_facts_on_file; true), % Ardillo: se si sta sfruttando la comunicazione per stringhe si scrivono i fatti su un file apposito.
	clean_dirtyrules, % MINAFRA
	clean_frame_fact,
	timediff. % Luigi Tedone
...
forward :-
	...
	!.
forward :- %verifica ogni fatto in ciascuna regola, sfruttando l'indicizzazione, dopo di che lo riscrive come used_fact
	fact(Idf,F,Cf),
	gui_forward(Idf,F),
	calculate_k,
...
checkframe([A|T]) :- %verifica che il framedata unifica con la condizione della regola
        nonvar(A),
        A \== call_p,
        not(number(A)),
        A =.. [Slot, Frame, Value],
...
/****************** Procedura checkFactInCond
Galetta F. 
Per velocizzare il processo inferenziale, è stata creata questa procedura che permette di controllare la presenza del fatto preso
in considerazione tra le condizioni della regola. Questo per evitare un ciclo di unificazione sull'albero che non porterà a nessun risultato utile.
***/
checkFactInCond(F,Tree) :- %Galetta F.
...
%aggiornamento contenuto variabile globale
asserting_head(Idr, set_global(N,V), Crt, Val, F) :-
    maxidg(M),
	IdgNew is M+1,
	idglist_upd(IdgNew),
	retractall(global(_,N,_)),
	assert(global(IdgNew,N,V)).
...
asserting_head(Idr, information_needed(Info_ID), Crt, Val, F) :-
	assert(info_need(Info_ID)).

% ---------------------------------------------------------------------------- %


%-----------------Inserito da De Filippis-Cafagna-------
%--------Nuova funzionalit�: se decommentata, abilita la possibilit� di asserire fatti multipli appositamente inseriti in una lista.
asserting_head([[]]).
asserting_head([[HL|TL]|T]) :- asserting_head(HL), asserting_head(TL), asserting_head(T).
%--------------------------------------------------------------------------------------------
...
% MINAFRA commentato per far SCRIVERE i fatti dedotti con certezza 0 per permettere la verifica di no(0)
...
check_unique(IdfNew,R,Crt) :-
	...
	assert(fact(id(Modulo,IdfNew),R,Crt)).

% asserisce il nuovo fatto quando il modulo iniziale non è presente	
check_unique(IdfNew,R,Crt) :-
	assert(fact(IdfNew,R,Crt)).
...
check_operation(R,IdfNew) :-
	...
	assert(fact(IdfNew,r(f,I),1)).
 check_operation(R,IdfNew) :-
 	op(I,X,[Y,Z], between)=R,
 	X>Y,
 	X<Z,
 	not(fact( _ , r(_, I), 1)),
 	assert(fact(IdfNew,r(t,I),1)).
check_operation(R,IdfNew) :-
	op(I,X,[Y,_Z], between)=R,
	X=<Y,
	not(fact( _ , r(_, I), 1)),
	assert(fact(IdfNew,r(f,I),1)).
check_operation(R,IdfNew) :-
	op(I,X,[_Y,Z], between)=R,
	X>=Z,
	not(fact( _ , r(_, I), 1)),
	assert(fact(IdfNew,r(f,I),1)).
check_operation(R,IdfNew) :-
	op(I,X,[Y,Z], between_eq)=R,
	X>=Y,
	X=<Z,
	not(fact( _ , r(_, I), 1)),
	assert(fact(IdfNew,r(t,I),1)).
check_operation(R,IdfNew) :-
	op(I,X,[Y,_Z], between_eq)=R,
	X<Y,
	not(fact( _ , r(_, I), 1)),
	assert(fact(IdfNew,r(f,I),1)).
check_operation(R,IdfNew) :-
 	op(I,X,[_Y,Z], between_eq)=R,
 	X>Z,
 	not(fact( _ , r(_, I), 1)),
 	assert(fact(IdfNew,r(f,I),1)).
check_operation(R,_) :-
...
%-----------------Variante De Filippis-Cafagna--------------------------------------------------
%----Assicurarsi che sia commentato per impedire la duplicazione delle regole nel caso di clausole multiple nella testa------
/*fireset_do(Idr,[R|L],C,P,Crt) :-
	...
	exp_rule(Idr,_,AC),
	%mennitti occhioner
	assert(exp_rule(IdrNew,R,C,_,AC)),
...
/* clean_dirtyrules
 elimina tutte le regole intermedie asserite durante il processo inferenziale
AUTORE: Danilo Minafra
***********************************/
clean_dirtyrules:-
