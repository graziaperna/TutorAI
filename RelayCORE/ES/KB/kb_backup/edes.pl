...
rule(42,mostra(X,criterio_diagnostico_paura_aumentare_peso),and([accusa(X,intensa_paura_di_aumentare_di_peso_o_dingrassare),accusa(X,comportamento_persistente_che_interferisce_con_laumento_di_peso,_nonostante_il_peso_sia_significativamente_basso)]),1,1).
...
rule(93,test(X),_and([presenta(X)]),1,1).
