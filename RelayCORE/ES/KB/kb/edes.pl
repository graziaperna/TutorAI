...
rule(9,manifesta(X,bed_disturbo_da_alimentazione_incontrollata_in_forma_lieve),and([mostra(X,comportamento_generale_2),mostra(X,criterio_diagnostico_abbuffate_incontrollate),mostra(X,criterio_diagnostico_mancanza_controllo_mangiare),not(mostra(X,criterio_diagnostico_comportamento_compenso)),mostra(X,criterio_diagnostico_abbuffata_convulsiva_media_da_1_a_3_volta_sett_per_6mesi),mostra(X,criterio_diagnostico_disagio_sofferenza_abbuffate_compulsive)]),1,1).
rule(10,manifesta(X,bed_disturbo_da_alimentazione_incontrollata_in_forma_moderata),and([mostra(X,comportamento_generale_2),mostra(X,criterio_diagnostico_abbuffate_incontrollate),mostra(X,criterio_diagnostico_mancanza_controllo_mangiare),not(mostra(X,criterio_diagnostico_comportamento_compenso)),mostra(X,criterio_diagnostico_abbuffata_convulsiva_media_da_4_a_7_volta_sett_per_6mesi),mostra(X,criterio_diagnostico_disagio_sofferenza_abbuffate_compulsive)]),1,1).
rule(11,manifesta(X,bed_disturbo_da_alimentazione_incontrollata_in_forma_grave),and([mostra(X,comportamento_generale_2),mostra(X,criterio_diagnostico_abbuffate_incontrollate),mostra(X,criterio_diagnostico_mancanza_controllo_mangiare),not(mostra(X,criterio_diagnostico_comportamento_compenso)),mostra(X,criterio_diagnostico_abbuffata_convulsiva_media_da_8_a_13_volta_sett_per_6mesi_),mostra(X,criterio_diagnostico_disagio_sofferenza_abbuffate_compulsive)]),1,1).
rule(12,manifesta(X,bed_disturbo_da_alimentazione_incontrollata_in_forma_gravissima),and([mostra(X,comportamento_generale_2),mostra(X,criterio_diagnostico_abbuffate_incontrollate),mostra(X,criterio_diagnostico_mancanza_controllo_mangiare),not(mostra(X,criterio_diagnostico_comportamento_compenso)),mostra(X,criterio_diagnostico_abbuffata_convulsiva_media_piu_di_14_volte_sett_per_6mesi),mostra(X,criterio_diagnostico_disagio_sofferenza_abbuffate_compulsive)]),1,1).
...
rule(14,manifesta(X,anoressia_riversa_vigoressia),and([mostra(X,comportamento_generale_4),mostra(X,fattore_caratterizzante_anomalia_percezione_peso_e_corpo),mostra(X,fattore_caratterizzante_abuso_integratori_diete),mostra(X,fattore_caratterizzante_esercizio_fisico_compulsivo)]),1,1).
...
rule(20,manifesta(X,anoressia_nervosa_con_restrizioni),and([mostra(X,comportamento_generale_09),mostra(X,criterio_diagnostico_basso_peso),mostra(X,criterio_diagnostico_paura_aumentare_peso),mostra(X,criterio_diagnostico_anomalia_percezione_peso_e_corpo),not(mostra(X,criterio_diagnostico_abbuffate_incontrollate)),not(mostra(X,criterio_diagnostico_comportamento_compenso)),not(ha(X,avuto_durante_gli_ultimi_3_mesi_episodi_di_abbuffate_o_comportamenti_di_eliminazione))]),1,1).
rule(21,manifesta(X,anoressia_nervosa_con_abbuffate_e/o_manovre_di_elinazione),and([mostra(X,comportamento_generale_10),mostra(X,criterio_diagnostico_basso_peso),mostra(X,criterio_diagnostico_paura_aumentare_peso),mostra(X,criterio_diagnostico_anomalia_percezione_peso_e_corpo),ha(X,avuto_negli_ultimi_3_mesi_ricorrenti_episodi_di_abbuffate_o_comportamenti_di_eliminazione)]),1,1).
rule(22,manifesta(X,anoressia_nervosa_atipica),and([not(mostra(X,comportamento_generale_0)),not(mostra(X,criterio_diagnostico_basso_peso)),not(mostra(X,criterio_diagnostico_paura_aumentare_peso)),mostra(X,criterio_diagnostico_anomalia_percezione_peso_e_corpo),mostra(X,bmi_regolare)]),1,1).
...
%robe
rule(41,mostra(X,criterio_diagnostico_basso_peso),and([mostra(X,bmi_sottopeso),accusa(X,restrizione_apporto_energetico_che_induce_un_significativo_basso_peso_ovvero_un_peso_minore_del_minimo_normale_o_per_i_bambini_e_gli_adolescenti_minore_del_minimo_atteso)]),1,1).
rule(42,mostra(X,criterio_diagnostico_paura_aumentare_peso),and([accusa(X,intensa_paura_di_aumentare_di_peso_o_dingrassare),accusa(X,comportamento_persistente_che_interferisce_con_laumento_di_peso_nonostante_il_peso_sia_significativamente_basso)]),1,1).
...
rule(48,mostra(X,criterio_diagnostico_abbuffata_convulsiva_media_da_1_a_3_volta_sett_per_6mesi),and([ha(X,abbuffate_compulsive_in_media_almeno_da_una_a_tre_volte_a_settimana_per_almeno_sei_mesi)]),1,1).
...
rule(50,mostra(X,criterio_diagnostico_assenza_interesse_cibo),and([presenta(X,assenza_di_interesse_per_lalimentazione_o_per_il_cibo),presenta(X,evitamento_basato_sulle_caratteristiche_sensoriali_del_cibo)]),1,1).
...
rule(53,mostra(X,fattore_caratterizzante_abuso_integratori_diete),and([presenta(X,abuso_di_integratori_anabolizzanti_e_diete_iperproteiche)]),1,1).
...
rule(55,mostra(X,criterio_diagnostico_no_condizione_medica_mentale),and([or([accusa(X,anomalia_non_meglio_attribuibile_a_una_condizione_medica_o_ad_un_altro_disturbo_mentale),presenta(X,comportamento_non_dovuto_ad_una_condizione_gastrointestinale_associata_o_ad_unaltra_condizione_medica_generale_per_es_reflusso_gastroesofageo_stenosi_del_piloro),accusa(X,comportamento_di_ingestione_si_manifesta_esclusivamente_durante_il_decorso_di_un_altro_disturbo_mentale_per_es_ritardo_mentale_disturbo_dello_spetto_autistico_schizofrenia_disturbo_pervasivo_dello_sviluppo_o_condizione_medica_gravidanza_compresa_e_sufficientemente_grave_da_giustificare_di_per_se_attenzione_clinica)])]),1,1).
...
rule(63,mostra(X,fattore_caratterizzante_evitamento_situzioni_sociali_cibo_non_controllato),and([presenta(X,evitamento_delle_situazioni_sociali_che_espongono_al_non_controllo_del_cibo)]),1,1).
...
rule(66,mostra(X,bmi_regolare),and([presenta(X,peso_regolare_attraverso_l_indice_di_massa_corporea_bmi)]),1,1).
rule(67,mostra(X,bmi_sovrappeso),and([presenta(X,sovrappeso_attraverso_l_indice_di_massa_corporea_bmi)]),1,1).
rule(68,mostra(X,bmi_sottopeso),and([presenta(X,sottopeso_attraverso_l_indice_di_massa_corporea_bmi)]),1,1).
...
rule(78,mostra(X,criterio_diagnostico_abbuffata_convulsiva_media_da_4_a_7_volta_sett_per_6mesi),and([ha(X,abbuffate_compulsive_in_media_almeno_da_quattro_a_sette_volte_a_settimana_per_almeno_sei_mesi)]),1,1).
rule(79,mostra(X,criterio_diagnostico_abbuffata_convulsiva_media_da_8_a_13_volta_sett_per_6mesi),and([ha(X,abbuffate_compulsive_in_media_almeno_da_otto_a_tredici_volte_a_settimana_per_almeno_sei_mesi)]),1,1).
rule(80,mostra(X,criterio_diagnostico_abbuffata_convulsiva_media_piu_di_14_volte_sett_per_6mesi),and([ha(X,abbuffate_compulsive_in_media_almeno_da_una_a_tre_volte_a_settimana_per_almeno_sei_mesi)]),1,1).
...
rule(92,mostra(X,vecchia_situzione_cura_dca),and([presenta(X,una_vecchia_situazione_di_cura_per_dca)]),1,1).

rule(93,test(X),and([presenta(X)]),1,1).
rule(94,manifesta(X,anoressia_nervosa),and([mostra(X,comportamento_generale_0),mostra(X,criterio_diagnostico_basso_peso),mostra(X,criterio_diagnostico_paura_aumentare_peso),mostra(X,criterio_diagnostico_anomalia_percezione_peso_e_corpo),mostra(X,no_indice_gravita_anoressia)]),1,1).
