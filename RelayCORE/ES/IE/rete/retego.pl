...
filter_x(_,X,_,X) :- not use_filter(yes), !.
filter_x(_,[],X,X).
filter_x(Max,[inst(N,[A/T|Z],C)|X],Temp,ML) :-
	T < Max,
	!, filter_x(Max,X,Temp,ML).
filter_x(Max,[inst(N,[A/T|Z],C)|X],Temp,ML) :-
	T = Max,
	!, filter_x(Max,X,[inst(N,[A/T|Z],C)|Temp],ML).
filter_x(Max,[inst(N,[A/T|Z],C)|X],Temp,ML) :-
	T > Max,
	!, filter_x(T,X,[inst(N,[A/T|Z],C)],ML).
sort_x(L,R) :-
	build_keys(L,LK),
	sort(LK,X),
	reverse(X,[K-R|_]).
...
sen(Node-n,tok(AD,Term)) :-
	retract(n(Node,T,NextList,Count)),
	(AD = add, NewCount is Count + 1 ;
	 AD = del, NewCount is Count - 1 ),
	asserta(n(Node,T,NextList,NewCount)),
	!,matchNot(Node,AD,Term,NextList).
...
% viene creato un nuovo token aggiungendo al token originale la parte sinistra della memoria.
% Il nuovo token viene inviato nella struttura con send
matchNot(Node,AD,TokenList,NextList) :-
	n(Node,_,_,0),
	(AD = add, send(tok(del,TokenList),NextList) ;
	 AD = del, send(tok(add,TokenList),NextList) ),
	fail.
matchNot(_,_,_,_).
