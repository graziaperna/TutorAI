...
or_deleter(N,RHS,[], K, P, NewLHS) :-
	gen_rnid(RNID),
	assert(rule(RNID,RHS,and(NewLHS),K,P)).
...
% SE CLAUSOLA E' NOT ALLORA ANDIAMO QUI	
...
rete_comp(N,[not(H)|T],RHS) :-
	check_root(RN,H,HList),
	check_not(NN,[H/_],NHList),
	update_root(RN,HList,NN-n),
	retcom(n(NN),[H/_],NHList,T,N,[RHS]),
	message(202,N), !.
...
% in caso contrario ne crea uno nuovo, caso in cui la nuova clausola è not
retcom(PNID,PrevNode,PrevList,[not(H)|T],N,RHS) :-
	check_root(RN,H,HList),
	check_not(NN,[H/_],NHlist),
	check_node(PrevNode,PrevList,[H/_],HList,NID,OutTok,NList),
	update_node(PNID,PrevList,NID-l),
	update_not(NN,NHlist,NID-r),
	update_root(RN,HList,NN-n),
	!,
...
%%%%%%%%%%% caso not
% crea un nodo not in quanto non c'è corrispondeza tra Term e i nodi radice già esistenti
check_not(NID,Term,[]) :-
	not(n(_,Term,_,_)),
	gen_nid(NID),
	assertz( n(NID,Term,[],0) ), !.

% verifica che il nodo radice relativo a Term esiste già nella struttura
check_not(N,Term,List) :-
	asserta(temp(Term)),
	retract(temp(T1)),
	n(N,Term,List,0),
	n(N,T2,_,0),
...
% non esiste una corrispondeza precisa e viene creato un nuovo nodo not
check_not(NID,Term,[]) :-
	gen_nid(NID),
	assertz( n(NID,Term,[],0) ).
...
update_not(RN,HList,NID) :-
	retract( n(RN,H,HList,0) ),
	asserta( n(RN,H,[NID|HList],0) ).
...
% richiama il predicato update_not nel caso in cui il nodo sia un nodo not
update_node(n(RN),HList,NID) :-
	update_not(RN,HList,NID), !.	
