...
writer_explain([UFact|FactTail], DedFact) :-
	used_fact(_,UFact,CertUFact),
	%explainer(UFact,FactExp,X),
	explainer(UFact,FactExp,X,_),
...
explaining_rule(IDIRule,CERTDed) :-
	%gtrace,
	exp_rule(IDIRule,DedFact,UsedFacts),
	not(explained_fact(DedFact)),
	%used_fact(_ , DedFact, _),
	%explainer(DedFact,DFactExp,X),%%%%%%%%%%%%%%%%%%%%%%%5
	explainer(DedFact,DFactExp,X,_),
...
explaining_rule2(IDIRule,CERTDed) :-
