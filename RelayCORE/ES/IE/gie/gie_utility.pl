...
:- dynamic idg/1.
:- dynamic global/3.
:- dynamic initglobals/1.
:- dynamic maxinitialidg/1.
:- dynamic maxidg_val/1.
:- dynamic fact/3.
:- dynamic rule/5.
:- dynamic used_fact/3.
:- dynamic used_rule/5.
...
%Mele: Aggiunge il nuovo id della variabile alla lista
idglist_upd(Idg) :-
	idg(L),
	append([Idg],L,Ln),
	retract(idg(L)),
	assert(idg(Ln)),
	retract(maxidg_val(_)),
	assert(maxidg_val(Idg)).
...
initialfacts :- % Aggiunta da Ardillo I. - caso in cui si carica un file di fatti vuoto.
	assert(maxinitialidf(0)),
	assert(maxidf_val(0)), 
	assert(fact(0, nil, 1)),!.
	
%Inizializzazione dei fatti di un modulo anonimo
initialfacts(fact(Idf, F, C)) :-
	initfacts(I),
	idf(L),
	append([Idf],L,Ln),
	retract(idf(L)),
...
%Mele: Crea la lista contenente gli id delle variabili globali iniziali
initialglobals :-
	global(Idg,_,_),
	initglobals(I),
	idg(L),
	...
	assert(idg(Ln)),
	append([Idg],I,In),
	retract(initglobals(I)),
	assert(initglobals(In)),	
	fail.

initialglobals :-
	initglobals(I),
	get_max(I,M),
	assert(maxinitialidg(M)),
	assert(maxidg_val(M)),!.
	
%Inizializzazione delle variabili globali di un modulo anonimo Mele
initialglobal(global(Idg, G, V)) :-
	initglobals(I),
	idg(L),
	append([Idg],L,Ln),
	retract(idg(L)),
...
%Inizializzazione delle variabili globali di un modulo con nome
initialglobals(ModuleName) :-
	global(id(ModuleName, N),_,_),
	initglobals(I),
	idg(L),
	append([id(ModuleName, N)],L,Ln),
	retract(idg(L)),
...
*************************************************************************/
write_facts_on_file :- 
	retract(used_fact(0, nil, 1)),
	fail.
write_facts_on_file :- 
	stricomm_facts_path(P),
	open(P,write,F),
	findall(fact(Id,Fact,C), used_fact(Id,Fact,C), L),
	write_list_of_facts_on_file(L, F),
	close(F).


write_list_of_facts_on_file([],_).
write_list_of_facts_on_file([H|T],F) :- term_string(H,S), string_concat(S,".",NS), writeln(F,NS),
										write_list_of_facts_on_file(T,F).
