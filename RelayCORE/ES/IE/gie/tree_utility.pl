...
/*	Procedure di Ivan Ardillo
	Viene passata a to_print/2 la lista delle condizioni di una regola per cercarvi 
	le eventuali write/writeln all'interno, prima utilizzando find_callp e poi la
	find_write.
*/
find_write(Idr, call_p(write(Str))) :- 
	assert(to_write(Idr, Str)).%, write(Str).
find_write(Idr, call_p(writeln(Str))) :- 
	assert(to_write((Idr, Str))).%, writeln(Str).

find_callp(Idr, [El|List]) :- %gtrace,
	functor(El, call_p, 1),
	find_write(Idr, El), 
	fail.
find_callp(Idr, [El|List]) :- find_callp(Idr, List).
find_callp(_, []).
to_print(Idr, and(ListCond)) :- %gtrace,
	find_callp(Idr, ListCond).
...
updating(Idr,R,C,P,Crt,B,AllCond) :-  
	...
	idrlist_upd(IdrNew),
	!,
	to_print(IdrNew, AllCond), % modifica di Ivan Ardillo: checking delle regole che eventualmente tentano una stampa a video.
