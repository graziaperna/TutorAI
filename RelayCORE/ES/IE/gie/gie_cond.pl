/* gie_cond.pl : Meccanismo di Gestione delle Condizioni
	Realizzazione: Settembre, 2013 (Nicola Milella)
		Descrizione: dati
	...
	Revisione: Febbraio, 2014 (Francesco Solare)
		Obiettivo: Snellimento e Pulizia del Codice
    Revisioni: Ottobre, 2014 (Morisco M. - Petrera M.)
	...
	Revisione: Ottobre, 2016 (Iovine A. - Lovascio C.)
...
% Condizione AND
updatecond(and([]),_,_,and(Nc),and(Nc)).
...
*************************************************************************/
updatecond(no(F),F,Crt,no(_),no(Crt)) :-
	!.
updatecond(no(X),_,_,no(_),no(X)) :-
	number(X),
	!.
updatecond(no(X),F,Crt,no(_),no(Cn)) :-
...
updatecond(not_exists(F),F,Crt,not_exists(_),not_exists(Crt)) :-
	!.
updatecond(not_exists(X),_,_,not_exists(_),not_exists(X)) :-
	number(X),
	!.
updatecond(not_exists(X),F,Crt,not_exists(_),not_exists(Cn)) :-
	updatecond(X,F,Crt,_,Cn),
	!.
updatecond(not_exists(X),_,_,not_exists(_),not_exists(X)) :-
	!.
...
% Condizione add
updatecond(add([]),_,_,add(Nc),B,add(Nc)) :-
	verify(add(Nc),B).
...
% Condizione MUL
updatecond(mul([]),_,_,mul(Nc),B,mul(Nc)) :-
	verify(mul(Nc),B).
...
% Condizione DIV
updatecond(div([]),_,_,div(Nc),B,div(Nc)) :-
	verify(div(Nc),B).
...
% Condizione MIN
updatecond(minus([]),_,_,minus(Nc),B,minus(Nc)) :-
	verify(minus(Nc),B).
...
*/
and([X|T]) :- % se X è un numero
	number(X),
	X=\=0,	
	X=<1,
	!,
	and(T).
and([X|T]) :- % se X è una sotto condizione di tipo OR
	functor(X,or,_),
	call(X),
	!,
	and(T).
and([X|T]) :- % se X è una sotto condizione di tipo AND
	functor(X,and,_),
	call(X),
	!,
	and(T).
and([X|T]) :- % se X è una sotto condizione di tipo NO
	functor(X,no,_),
	call(X),
	!,
	and(T).
...
add([]).
add([X|T]) :- % se X è un numero
	number(X),
	!,
	add(T).
add([X|T]) :- % se X è una sotto condizione di tipo OR
	functor(X,or,_),
	call(X),
	!,
	add(T).
add([X|T]) :- % se X è una sotto condizione di tipo AND
	functor(X,and,_),
	call(X),
	!,
	add(T).
add([X|T]) :- % se X è una sotto condizione di tipo NO
	functor(X,no,_),
	call(X),
	!,
	add(T).
...
mul([]).
mul([X|T]) :- % se X è un numero
	number(X),	
	!,
	mul(T).
mul([X|T]) :- % se X è una sotto condizione di tipo OR
	functor(X,or,_),
	call(X),
	!,
	mul(T).
mul([X|T]) :- % se X è una sotto condizione di tipo AND
	functor(X,and,_),
	call(X),
	!,
	mul(T).
mul([X|T]) :- % se X è una sotto condizione di tipo NO
	functor(X,no,_),
	call(X),
	!,
	mul(T).
...
div([]).
div([X|T]) :- % se X è un numero
	number(X),
	!,
	div(T).
div([X|T]) :- % se X è una sotto condizione di tipo OR
	functor(X,or,_),
	call(X),
	!,
	div(T).
div([X|T]) :- % se X è una sotto condizione di tipo AND
	functor(X,and,_),
	call(X),
	!,
	div(T).
div([X|T]) :- % se X è una sotto condizione di tipo NO
	functor(X,no,_),
	call(X),
	!,
	div(T).
...
minus([]).
minus([X|T]) :- % se X è un numero
	number(X),
	!,
	minus(T).
minus([X|T]) :- % se X è una sotto condizione di tipo OR
	functor(X,or,_),
	call(X),
	!,
	minus(T).
minus([X|T]) :- % se X è una sotto condizione di tipo AND
	functor(X,and,_),
	call(X),
	!,
	minus(T).
minus([X|T]) :- % se X è una sotto condizione di tipo NO
	functor(X,no,_),
	call(X),
	!,
	minus(T).
