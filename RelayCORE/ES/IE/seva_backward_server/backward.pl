...
esplora(not(X), Catena, Prob-not(Traccia)) :- % negazione
	!,
	%retract(fact_not_found(_)),
	%assert(fact_not_found(1)),
	esplora_not(X, Catena, Traccia, Prob).
	%fact_not_found(0).
...
esplora_not(X, Traccia1, Prob-Traccia, ProbNeg) :- % X è vero
	esplora(X, Traccia1, Prob-Traccia),
	!,
	ProbNeg is 1 - Prob,
	fail_check(ProbNeg).
esplora_not(X, _Traccia1, failed(X), 1.0). % X è falso
